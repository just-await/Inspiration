...
// --- НАСТРОЙКА SUPABASE ---
// 1. Вставь ссылку из раздела Project URL
const SUPABASE_URL = 'https://brinoaifolxiuyczysfh.supabase.co'; 

// 2. Вставь ключ из раздела Publishable key (default)
...
// Теперь в userSessionId лежит вечный паспорт этого браузера
...
quoteForm.addEventListener('submit', async (e) => {
    ...
    // Берем контакт (удаляем @ если пользователь его ввел)
    ...
    try {
        ...
        // Обновляем ссылку на бота с паспортом
        const botLinkBtn = successStep.querySelector('a');
        // Замени тут 'MyInspoMod_bot' на юзернейм своего бота!
...
(async () => {
    // 1. Сначала загружаем базу
    await fillQuotePool();
    
    // 2. Выбираем цитату
    currentQuoteObj = await getSmartQuoteObj();
    
    // 3. Вставляем текст, НО блок пока прозрачный (initial-hidden)
    quoteText.textContent = currentQuoteObj.text;
    if (currentQuoteObj.author) {
        quoteAuthor.textContent = `© ${currentQuoteObj.author}`;
    }

    // 4. Небольшая задержка, чтобы браузер понял, что блок скрыт
    // и подготовился к красивой анимации (500мс идеально)
    setTimeout(() => {
